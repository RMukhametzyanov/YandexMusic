...
class PlayRadio(BasePage):
    ...
    def delete_song(self):
        with allure.step("Поиск песни в плейлисте"):
            name_song_for_delete = self.find_element(PlayRadioPageLocators.LOCATOR_LikeSongNameInMyPlaylist)
        ...
        with allure.step("Поиск и клик по кнопке 'Удаление'"):
            delete_song = self.find_element(PlayRadioPageLocators.LOCATOR_DeleteSong_button)
    ...
    def check_song_deleted(self):
        with allure.step("Проверяем наличие 'Плейлист пока пуст'"):
            ...
            element_success = self.find_element(PlayRadioPageLocators.LOCATOR_PlaylistEmpty).text
